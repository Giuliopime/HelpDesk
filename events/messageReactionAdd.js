...
module.exports = async (client, reaction, user) =>  {
    try {
        ...
        if (hdChannels.includes(message.channel.id)) {
            ...
            if(!message.channel.permissionsFor(guild.me.id).has(requirePerms)) {
                return message.channel.send(message.client.errorEmbed.setDescription(`I'm missing some important permissions to manage the help-desk>:\n**>** ${requirePerms.filter(permission => !message.channel.permissionsFor(message.guild.me.id).has(permission)).join('\n**>** ')}`)).catch();
            }
                ...
                if (fieldIndex === 9) {
                    ...
                    if (roleToAssign) {
                        member.roles.add(roleToAssign.id)
                            ...
                            .catch(() => {
                                client.errorEmbed.setDescription(`I\'m unable to assign the <@&${roleToAssign.id}> role. If you are a moderator of the server make sure I have \`Manage_Roles\` permissions and that the role I have to assign is under the Help Desk role in the server role hierarchy.\nLearn more with [this article](https://support.discord.com/hc/en-us/articles/214836687-Gestione-dei-Ruoli-101).\n\nIf you are not a moderator of the server report this error to one of them.`);
